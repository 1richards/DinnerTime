...
export default function ShoppingScreen() {
  ...
  // Share path replaces the Instacart handoff for v1.0 (the Instacart
  // integration is parked for post-MVP — see 999.x backlog). Opens the
  // iOS share sheet with a formatted plain-text list of unchecked items.
  // From the share sheet the user can route to Notes, Messages, Mail,
  // Reminders, Copy, AirDrop — whatever fits how they want to shop.
  const handleShare = useCallback(async () => {
    setSharing(true);
    try {
      const result = await shareShoppingListAsText(currentList, items);
      if (!result.ok && result.reason && result.reason !== 'dismissed') {
        show('Couldn’t open the share sheet — try again.', 'error');
      }
    } finally {
      setSharing(false);
    }
  }, [currentList, items, show]);

  const checkedCount = items.filter((i) => i.checked).length;
  const allChecked = items.length > 0 && items.every((i) => i.checked);
  const shareDisabled = sharing || items.length === 0 || allChecked;
